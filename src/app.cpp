		...
		for (b2ContactEdge *edge = game_->get_player()[i].get_body()->GetContactList(); edge; edge = edge->next) {
			if (edge->other->GetType() == b2BodyType::b2_dynamicBody) {
				if (edge->other->IsBullet()) {
					...
					edge->other->SetTransform(b2Vec2(-100,-100), 0);
					crashsound_.play();
...
void App::game_loop(unsigned planeIndex , unsigned mapIndex)
{
	...
	bool running = true;
	sf::Texture player1;
	player1.loadFromFile(planes[planeIndex-1]);
	sf::Sprite player1spr;
	player1spr.setTexture(player1);
	sf::Texture map;
	map.loadFromFile(maps[mapIndex-1]);
...
void App::show_instructions(){
    ...
    if (!instrtext.loadFromFile("images/instructions.png")) {
...
void App::app_main(){
	...
	planes.push_back("images/plane1_correct.png");
	planes.push_back("images/plane2_correct.png");
	...
	unsigned planeIndex = 1 , mapIndex = 1, playerIndex = 1;
    while(main_window.isOpen()){
        int ret = main_menu();
        if (ret == 0 || planeIndex == 0 || mapIndex == 0 || playerIndex == 0 ){
            main_window.close();
        }
...
unsigned App::selectPlane() {
	...
	vecStr.push_back("Blue");
	vecStr.push_back("Green");
	vecStr.push_back("Black");
	vecStr.push_back("Exit");
		...
		while (main_window.pollEvent(event))
		{
			...
			case sf::Event::KeyReleased:
				...
				case sf::Keyboard::Return:
					...
					{
					case 0:
						std::cout << "Red Biplane" << std::endl;
						return 1;
						break;

					case 1:
						std::cout << "Blue Biplane" << std::endl;
						return 2;
						break;

					case 2:
						std::cout << "Green Jet" << std::endl;
						return 3;
						break;
					case 3:
						std::cout << "Black Jet" << std::endl;
						return 4;
						break;
					case 4:
						std::cout << "Close button has been pressed" << std::endl;
