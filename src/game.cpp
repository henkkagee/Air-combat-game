...
void Game::init(unsigned mapIndex)
{
	planecount_ = 0;
	std::vector<int> x = { 5, 120 };
	std::vector<int> y = { 5, 20 };
	int player_count = 2;
	const char* name = "maps/map3_physics.bmp";
	if (mapIndex == 1) {
			name = "maps/map3_physics.bmp";
	}
	else if (mapIndex == 2) {
			name = "maps/map4_physics.bmp";
	}
	else if (mapIndex == 3) {
			name = "maps/map1_physics.bmp";
	}
	else if (mapIndex == 4) {
			name = "maps/map2_physics.bmp";
	}
...
void Game::objectify_map()			// create static terrain segments from bmp image
{
	...
	unsigned int b = 0;
	bool set = false;
	while (a < gamemap_[0].size() - 1) { 	// check ground level to set correct height
		if (set == true) {
			break;
		}
		b = 0;
		while (b < gamemap_.size() - 1) {
...
std::vector<std::vector<Pixel>> Game::loadmap(const char* filename)					// loads 24-bit BMP image file into game as map
{
	...
	uint32_t width = *(unsigned long*)&header[18];								// typecast to unsigned long-pointer and dereference to read 4 bytes at once
	uint32_t height = *(unsigned long*)&header[22];
	uint32_t padding = width % 4;												// The size of BMP rows are rounded up to a multiple of 4 by padding
